	...
	$output .= '<div class="dashboard_wrapper">';
		$output .= '<div class="logo_big"></div>';
			...
			foreach($get_forms as $form)
				$output .= '<div class="form_holder form"><div class="form_name" ><a href="'.get_option('siteurl').'/wp-admin/admin.php?page=WA-x_forms_express-main&form_id='.$form->Id.'">'.$form->title.'</a></div><div class="form_entry_count">('.XFormsExpress::get_total_form_entries($form->Id).')</div><div style="clear:both;"></div></div>';
		$output .= '</div>';
	$output .= '</div>';
...
function XFormsExpress_dashboard_setup() {
	...
	wp_add_dashboard_widget('XFormsExpress_dashboard_widget', 'X Forms', 'XFormsExpress_dashboard_widget');
