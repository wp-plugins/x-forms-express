...
$output .= '<body onLoad="tinyMCEPopup.executeOnLoad(\'init();\');">';
		$output .= '<div class="wrap">';
			$output .= '<h2>X Forms</h2>';
			$output .= '<form name="add_gallery_to_post" action="">';
				$output .= '<table cellpadding="5" cellspacing="1" width="100%" bgcolor="#CCC" style="font-size:11px;">';
					$output .= '<tr>';
						...
						$output .= '</td>';
						$output .= '<td bgcolor="#F2F2F2" width="30%">';
							$output .= build_dropdown('wap_x_forms').'<h3 style="float:left;">';
						$output .= '</td>';
					$output .= '</tr>';								
				$output .= '</table>';
				$output .= '<br><input type="button" name="Insert form" value="   Insert into post   " onClick="insert_x_form();">';		
			$output .= '</form>';
	$output .= '</div>';
$output .= '</body>';
echo $output;
