...
function()
	...
	jQuery('.opened_forms li.form').live('click',
		function()
			...
			var data =
				...
				jQuery('div.form_entry_view').html('Loading form entries...');
				jQuery('.tabs.ve_from_entries .tab_description').html('-- entries');
				jQuery.post
					(
					ajaxurl, data, function(response)
						{
						jQuery('div.form_entry_view').html(response);
						jQuery('.tabs.ve_from_entries .tab_description').html(jQuery('span.displaying-num').html());
	...
	jQuery('select[name="change_color_scheme"]').change(
		function()
			{
			jQuery(this).parent().find('link.color_scheme').attr('href',jQuery('#site_url').text() + '/wp-content/plugins/x-forms-express/css/'+ jQuery(this).val() +'/jquery.ui.theme.css');	
			}
		);
